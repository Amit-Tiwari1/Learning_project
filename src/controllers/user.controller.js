...
export const registerUser = async (req, res) => {
  try {
    ...
    const existingUser = await User.findOne({ where: { user_Email: newUser.user_Email } });
    ...
    return res.status(200).json(apiResponse({ email: user.user_Email }, "OTP has been sent to your registered email!"));
  } catch (error) {
...
export const authenticateUser = async (req, res) => {
  ...
  try {
    ...
    if (isVerified) {
      ...
      await User.update({ isAuthenticated: true, otp: null }, { where: { user_Id: userId } });
...
export const getAll = async (req,res) =>{
  ...
  try {
    const user_Data = await User.findOne({where:{user_Email:userData.user_Email}})
